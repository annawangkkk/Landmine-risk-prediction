...
cdef class Pnorm(ClassificationCriterion):
    cdef double node_impurity(self) nogil:
        return log(2)
    ...
    cdef void children_impurity(self, double* impurity_left, 
        ...
        cdef double p = 4.0
        cdef double pnorm = 0.0
        
        cdef double count_left_neg
        cdef double count_left_pos
        cdef double count_right_neg
        cdef double count_right_pos

        cdef double left_p_prob
        cdef double left_n_prob
        cdef double right_p_prob
        cdef double right_n_prob

        cdef double pos_counts
        cdef double neg_counts

        cdef SIZE_t r

        cdef double pos_sum
            
        count_left_neg = self.sum_left[0, 0] # sum[n_outputs, n_classes[k]]
        count_left_pos = self.sum_left[0, 1]
        count_right_neg = self.sum_right[0, 0]
        count_right_pos = self.sum_right[0, 1]

        left_prob = count_left_pos/(count_left_neg + count_left_pos)
        right_prob = count_right_pos/(count_right_neg + count_right_pos)
        
        neg_counts = self.sum_total[0,0]
        pos_counts = self.sum_total[0,1]
        
        if (neg_counts == 0) or (pos_counts == 0):
            impurity_left[0] = 0
            impurity_right[0] = 0
        else:
            pnorm += (((count_left_pos * log(2) 
                    + count_right_pos * log(1.0+exp(left_prob - right_prob)))/pos_counts)**p)*count_left_neg
            pnorm += (((count_left_pos * log(1.0+exp(right_prob - left_prob)) 
                    + count_right_pos * log(2))/pos_counts)**p)*count_right_neg
            pnorm = (pnorm/neg_counts)**(1/p) 
            impurity_left[0] = pnorm
            impurity_right[0] = pnorm
...
cdef class AUC(ClassificationCriterion):
    ...
    cdef void argsort(self, double* arr, int* indices, int n) nogil:
        cdef int i, j, tmp_idx
        cdef double tmp
    ...
    cdef double node_impurity(self) nogil:
        ...
        cdef double auc = 0.0
        cdef double ei_minus
        cdef double ei_plus
        cdef double ei_plus_prefix
        cdef double x_t
        cdef double y_t
        ...
        cdef int* opt_order = <int*>malloc(self.n_outputs * sizeof(int))
        cdef double* all_loc_acc = <double*>malloc(self.n_outputs * sizeof(double))
        
        # n_classes = [Leaf 1, Leaf 2, ...] * [(+,-),(+,-), ...]

        for k in range(self.n_outputs):
            opt_order[k] = k # init
            all_loc_acc[k] = self.sum_total[k,1]/(self.sum_total[k,0] + self.sum_total[k,1])
            x_t += self.sum_total[k,0] # neg  
            y_t += self.sum_total[k,1] # pos    
        
        self.argsort(all_loc_acc, opt_order, self.n_outputs) # optimal order of leaves (n_outputs)

        for k in range(self.n_outputs):
            ei_minus = self.sum_total[opt_order[k],0]
            ei_plus = self.sum_total[opt_order[k],1]
            for j in range(k-1):
                ei_plus_prefix += 2*self.sum_total[opt_order[j],1]
            auc += ei_minus*(ei_plus_prefix + ei_plus)
        ...
        if (x_t*y_t) == 0:
            ...
            with gil:
                print("parent auc",auc / (2*x_t*y_t))
            return -auc / (2*x_t*y_t)
    ...
    cdef void children_impurity(self, double* impurity_left,
        ...
        cdef double auc_left = 0.0
        cdef double auc_right = 0.0
        cdef double ei_minus
        cdef double ei_plus
        cdef double ei_plus_prefix
        cdef double x_t
        ...
        cdef int* opt_order = <int*>malloc(self.n_outputs * 2 * sizeof(int))
        cdef double* all_loc_acc = <double*>malloc(self.n_outputs *  2* sizeof(double))

        
        for k in range(self.n_outputs*2):
            opt_order[k] = k 
            if k < self.n_outputs:
                all_loc_acc[k] = self.sum_left[k,1]/(self.sum_left[k,0] + self.sum_left[k,1])
                x_t += self.sum_left[k,0] # neg  
                y_t += self.sum_left[k,1] # pos   
            else:
                all_loc_acc[k] = self.sum_right[k-self.n_outputs,1]/(self.sum_right[k-self.n_outputs,0] + self.sum_right[k-self.n_outputs,1])
                x_t += self.sum_right[k-self.n_outputs,0] # neg  
                y_t += self.sum_right[k-self.n_outputs,1]
        
        self.argsort(all_loc_acc, opt_order, 2*self.n_outputs)

        for k in range(self.n_outputs*2): # self.n_outputs == 1
            if (opt_order[k]) < self.n_outputs:
                ei_minus = self.sum_left[opt_order[k],0]
                ei_plus = self.sum_left[opt_order[k],1]
                for j in range(k-1):
                    ei_plus_prefix += 2*self.sum_left[opt_order[j],1]
            else:
                ei_minus = self.sum_right[opt_order[k] - self.n_outputs,0]
                ei_plus = self.sum_right[opt_order[k] - self.n_outputs,1]
                for j in range(k-1-self.n_outputs):
                    ei_plus_prefix += 2*self.sum_right[opt_order[j],1]
            
            auc_left += ei_minus*(ei_plus_prefix + ei_plus)
            auc_right += ei_minus*(ei_plus_prefix + ei_plus)
        ...
        if x_t*y_t == 0:
            ...
            with gil:
                print("left", auc_left / (2*x_t*y_t))
                print("right", auc_right / (2*x_t*y_t))
            impurity_left[0] = -auc_left / (2*x_t*y_t)
            impurity_right[0] = -auc_right / (2*x_t*y_t)


cdef class pAUC(ClassificationCriterion):
    ...
    cdef double node_impurity(self) nogil:
        ...
        for k in range(self.n_outputs):
            ...
            y_t += self.sum_total[k,1] # pos    
        # with gil:
        #     print("parent",np.asarray(self.sum_total))
        self.argsort(all_loc_acc, opt_order, self.n_outputs) # optimal order of leaves (n_outputs)

        for k in range(int(self.n_outputs/2)):
            ei_minus = self.sum_total[opt_order[k],0]
            ei_plus = self.sum_total[opt_order[k],1]
            for j in range(k-1):
                ei_plus_prefix += 2*self.sum_total[j,1]
            auc += ei_minus*(ei_plus_prefix + ei_plus)
        
        free(all_loc_acc)
        free(opt_order)

        # if (x_t*y_t) == 0: # Half of AUC
        #     return 0.001
        # else:
        #     return 0.001
        return 0.001
    ...
    cdef void children_impurity(self, double* impurity_left,
        ...
        # with gil:
        #     print("left", np.asarray(self.sum_left))
        #     print("right", np.asarray(self.sum_right))
        for k in range(self.n_outputs*2):
            opt_order[k] = k 
            if k < self.n_outputs:
                all_loc_acc[k] = self.sum_left[k,1]/(self.sum_left[k,0] + self.sum_left[k,1])
        ...
        for k in range(self.n_outputs): # self.n_outputs == 1, Only difference here with raw AUC!
            if (opt_order[k]) < self.n_outputs:
                ei_minus = self.sum_left[opt_order[k],0]
                ei_plus = self.sum_left[opt_order[k],1]
                for j in range(k-1):
                    ei_plus_prefix += 2*self.sum_left[j,1]
                auc_left += ei_minus*(ei_plus_prefix + ei_plus)
            else:
                ei_minus = self.sum_right[opt_order[k] - self.n_outputs,0]
                ei_plus = self.sum_right[opt_order[k] - self.n_outputs,1]
                for j in range(k-1-self.n_outputs):
                    ei_plus_prefix += 2*self.sum_right[j,1]
                auc_right += ei_minus*(ei_plus_prefix + ei_plus)
        ...
        # with gil:
        #     if (x_t*y_t) != 0:
        #         print("auc_left", auc_left / (2*x_t*y_t))
        #         print("auc_right", auc_right / (2*x_t*y_t))
        
        if x_t*y_t == 0:
            impurity_left[0] = 0.001
            impurity_right[0] = 0.001
        else:
            impurity_left[0] = auc_left / (2*x_t*y_t)
            impurity_right[0] = auc_right / (2*x_t*y_t)
