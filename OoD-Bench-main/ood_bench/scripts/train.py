...
import sys
sys.path.append('/home/siqiz/Landmine/OoD-Bench-main/external')
...
def evaluate(model: nn.Module, zipped_minibatches: Iterable, device: str) -> float:
    ...
    for i, minibatches in enumerate(zipped_minibatches):
        x, y, _, _ = map(torch.cat, zip(*minibatches)) # RELand
        e = torch.cat([torch.zeros(x.size(0) // 2, dtype=torch.long),
                       torch.ones (x.size(0) // 2, dtype=torch.long)])
        x = x.to(device)
        y = F.one_hot(y.long(), num_classes=model.class_dim).float().to(device)
        e = e.to(device)
...
if __name__ == '__main__':
    ...
    if args.dataset == 'RELand':
        dataset = dataset_class(municipio=args.split, 
                                valid_city=args.valid,
                                subset=args.subset)
    else:
    ...
    for epoch in range(1, args.n_epochs + 1):
        ...
        for i, minibatches in enumerate(zip(in_dataloader_p, in_dataloader_q)):
            model.train()
            x, y, _, _ = map(torch.cat, zip(*minibatches))
