...
def get_backbone(name: str, input_shape: Tuple[int, int, int],
    ...
    elif name == 'mlp-reland':
        backbone = RELand_MLP(input_shape)
...
class RELand_MLP(Backbone):
    def __init__(self, input_shape):
        self._hdim = 20
        self.in_features = input_shape[0] # check which dimension
        super(RELand_MLP, self).__init__()
    ...
    def _load_modules(self):
        self.modules_ = nn.Sequential(nn.Linear(self.in_features, self._hdim), 
                                   nn.BatchNorm1d(self._hdim),
                                   nn.ReLU())
