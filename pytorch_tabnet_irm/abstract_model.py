...
class TabModel(BaseEstimator):
    ...
    def fit(
        ...
        train_dataloader, valid_dataloaders = self._construct_loaders(
            X_train, y_train, hist_mine, eval_set
        )
    ...
    def _train_epoch(self, train_loader):
        ...
        for batch_idx, (X, y, hist_mine) in enumerate(train_loader):
            self._callback_container.on_batch_begin(batch_idx)

            batch_logs = self._train_batch(X, y, hist_mine)
    ...
    def _train_batch(self, X, y, hist_mine):
        """
        ...
        # IRM related
        def make_environment(hist_mine, data, target):
            # env == hard to predict (unrelated to hist_mines)
            # env == easy to predict (related to hist_mines)
            # only give 2 environments
            ...
            if len(hard) == 0:
                res = [(data[easy_all[:len(easy_all)//2]], target[easy_all[:len(easy_all)//2]]),
                    (data[easy_all[len(easy_all)//2:]], target[easy_all[len(easy_all)//2:]])]
            if len(easy_all) == 0:
                res = [(data[hard[:len(hard)//2]], target[hard[:len(hard)//2]]),
                    (data[hard[len(hard)//2:]], target[hard[len(hard)//2:]])]
            # if we don't reweight hard-easy samples
        ...
        hist_mine = hist_mine.to(self.device).float()
        ...
        lambda1 = 5e-4
        envs = make_environment(hist_mine, X, y)
        loss_ce_envs = []
        loss_irm_envs = []
        for env in envs:
            micro_tx, micro_target = env
            out_micro, _ = self.network(micro_tx)
            out_micro = out_micro[:,1].squeeze(-1) # pick positive
            loss_ce_micro = nn.BCEWithLogitsLoss()(out_micro, micro_target)
            loss_irm_micro = penalty(out_micro, micro_target)
        ...
        for k, param in self.network.state_dict().items():
            if (param.requires_grad):
                weight_norm += param.norm().pow(2)
        loss = lambda1 * weight_norm + loss_ce + loss_irm 
    ...
    def _construct_loaders(self, X_train, y_train, hist_mine, eval_set):
