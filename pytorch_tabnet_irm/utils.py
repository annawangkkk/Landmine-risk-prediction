...
class TorchDataset(Dataset):
    ...
    def __init__(self, x, y, hist_mine):
        self.x = x
        self.y = y
    ...
    def __getitem__(self, index):
        ...
        return x, y, self.hist_mine[index]
...
def create_dataloaders(
    X_train, y_train, hist_mine, eval_set, weights, batch_size, num_workers, drop_last, pin_memory
):
    """
    ...
    train_dataloader = DataLoader(
        TorchDataset(X_train.astype(np.float32), y_train, hist_mine),
    ...
    for X, y in eval_set:
        valid_dataloaders.append(
            DataLoader(
                TorchDataset(X.astype(np.float32), y, hist_mine), # hist_mine for train but doesn't matter, not used for inference
                batch_size=batch_size,
                shuffle=False,
                num_workers=num_workers,
                pin_memory=pin_memory,
